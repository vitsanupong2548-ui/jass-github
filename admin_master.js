...
// ==========================================
// ส่วนจัดการตั๋ว (Ticket) ในหน้าสร้าง Event
// ==========================================
function renderPendingTickets() {
    ...
    pendingTickets.forEach((t, i) => { 
        ...
        <div class="bg-white border border-gray-200 p-3 rounded-lg flex justify-between items-center shadow-sm">
            <div>
                <span class="font-bold text-blue-600">${t.title}</span> 
                <span class="text-sm text-gray-500">(${parseFloat(t.price).toLocaleString()} THB / ${t.amount} ใบ)</span>
            </div>
            <div class="flex gap-2">
                <button type="button" onclick="editPendingTicket(${i})" class="text-blue-600 hover:text-white bg-blue-50 hover:bg-blue-500 border border-blue-200 px-3 py-1 rounded-md font-bold transition shadow-sm text-sm">แก้ไข</button>
                <button type="button" onclick="removePendingTicket(${i})" class="text-red-500 hover:text-white bg-red-50 hover:bg-red-500 border border-red-200 px-3 py-1 rounded-md font-bold transition shadow-sm text-sm">ลบ</button>
            </div>
        </div>`; 
    });
}

window.removePendingTicket = (i) => { 
    pendingTickets.splice(i, 1); 
    renderPendingTickets(); 
};

// 🌟 ฟังก์ชันใหม่: ดึงข้อมูลตั๋วกลับไปแก้ไข
window.editPendingTicket = (index) => {
    const t = pendingTickets[index];
    if (!t) return;

    // นำข้อมูลกลับไปใส่ช่องกรอก
    document.getElementById('temp_ticket_title').value = t.title || '';
    document.getElementById('temp_ticket_details').value = t.details || '';
    document.getElementById('temp_ticket_price').value = t.price || '';
    document.getElementById('temp_ticket_amount').value = t.amount || '';
    
    const statusRadios = document.querySelectorAll('input[name="temp_ticket_status"]');
    statusRadios.forEach(radio => {
        if (radio.value == t.status) radio.checked = true;
    });

    // เลื่อนหน้าจอไปที่ช่องกรอก
    document.getElementById('temp_ticket_title').scrollIntoView({ behavior: 'smooth', block: 'center' });

    // ลบออกจากคิว (เพื่อให้กด + Add ใหม่แล้วไม่ซ้ำ)
    pendingTickets.splice(index, 1);
    renderPendingTickets();
    
    showToast('ดึงข้อมูลมาแก้ไขแล้ว เมื่อแก้เสร็จให้กด + Add Ticket อีกครั้ง');
};

// ==========================================
// ส่วนจัดการศิลปิน (Line Up) ในหน้าสร้าง Event
// ==========================================
...
function renderPendingLineups() {
    ...
    pendingLineups.forEach((l, i) => { 
        c.innerHTML += `
        <div class="bg-white border border-gray-200 p-3 rounded-lg flex justify-between items-center shadow-sm">
            <div class="flex items-center gap-3">
                <div class="bg-black text-white text-xs px-2 py-1 rounded text-center">
                    <div>${l.date || '-'}</div>
                    <div class="text-yellow-400 font-bold">${l.time || '??:??'}</div>
                </div>
                <div>
                    <span class="font-bold text-blue-900">${l.name}</span> 
                    <span class="bg-gray-200 text-xs px-2 rounded ml-2">${l.stage}</span>
                </div>
            </div>
            <div class="flex gap-2">
                <button type="button" onclick="editPendingLineup(${i})" class="text-blue-600 hover:text-white bg-blue-50 hover:bg-blue-500 border border-blue-200 px-3 py-1 rounded-md font-bold transition shadow-sm text-sm">แก้ไข</button>
                <button type="button" onclick="removePendingLineup(${i})" class="text-red-500 hover:text-white bg-red-50 hover:bg-red-500 border border-red-200 px-3 py-1 rounded-md font-bold transition shadow-sm text-sm">ลบ</button>
            </div>
        </div>`; 
    });
}

window.removePendingLineup = (i) => { 
    pendingLineups.splice(i, 1); 
    renderPendingLineups(); 
};

// 🌟 ฟังก์ชันใหม่: ดึงข้อมูล Line Up กลับไปแก้ไข
window.editPendingLineup = (index) => {
    const l = pendingLineups[index];
    if (!l) return;

    document.getElementById('temp_lineup_date').value = l.date || '';
    document.getElementById('temp_lineup_time').value = l.time || '';
    document.getElementById('temp_lineup_stage').value = l.stage || '';
    document.getElementById('temp_lineup_name').value = l.name || '';

    document.getElementById('temp_lineup_date').scrollIntoView({ behavior: 'smooth', block: 'center' });

    pendingLineups.splice(index, 1);
    renderPendingLineups();
    
    showToast('ดึงข้อมูลมาแก้ไขแล้ว เมื่อแก้เสร็จให้กด + Add Line Up อีกครั้ง');
};
...
window.clickCourseSlot = async (slotNum, id) => {
    ...
    setTimeout(() => {
        ...
        if (window.courseBuilderSortable) {
            window.courseBuilderSortable.destroy(); // เคลียร์อันเก่าออกก่อน
        }
        if (contentContainer) {
            window.courseBuilderSortable = new Sortable(contentContainer, {
                animation: 150, // ความพริ้วตอนสลับที่
                handle: '.block-drag-handle', // ลากได้เฉพาะตรงไอคอนรูปมือ
...
function renderReviewTable(reviewsArray) {
    ...
    if (!tbody) return;
    if (countBadge) countBadge.textContent = `${reviewsArray.length} รายการ`;

    tbody.innerHTML = '';
    selectedReviewId = null; updateReviewActionButtons();
    ...
    reviewsArray.forEach(r => {
        ...
        let stars = '★'.repeat(ratingNum) + '☆'.repeat(5 - ratingNum);

        tr.innerHTML = `
            <td class="p-4 text-center"><input type="radio" name="review_selector" class="w-4 h-4 text-green-600 border-gray-300 pointer-events-none"></td>
            <td class="p-4">
                <div class="font-bold text-gray-800">${r.reviewer_name}</div>
                <div class="text-yellow-500 text-xs mt-1 tracking-widest">${stars}</div>
            </td>
...
document.getElementById('btn-action-edit-review')?.addEventListener('click', () => {
    ...
    if (r) {
        ...
        document.getElementById('review-rating').value = r.rating || 5; // ดึงดาว
...
window.cancelEditReview = () => {
    ...
    document.getElementById('review-rating').value = '5'; // รีเซ็ตดาว
...
window.saveCourseReview = async () => {
    ...
    const reviewRating = document.getElementById('review-rating').value; // ดึงดาว

    if (!reviewerName || !reviewText) return showToast('กรุณากรอกชื่อและเนื้อหารีวิวให้ครบ');
    ...
    fd.append('rating', reviewRating); // ส่งดาว

    try {
        const res = await fetch(getApiUrl('save_course_review'), fetchOptions('POST', fd));
        const result = await res.json();
        showToast(result.message);
        if (result.status === 'success') { document.getElementById('review-search-input').value = ''; fetchCourseReviews(); cancelEditReview(); }
    } catch (e) { }
};
...
document.getElementById('btn-action-edit-review')?.addEventListener('click', () => {
    ...
    if (r) {
        ...
        document.getElementById('review-name').value = r.reviewer_name;
        document.getElementById('review-text').value = r.review_text;
        document.getElementById('review-rating').value = r.rating || 5; // ดึงค่าดาวเดิมมาใส่ฟอร์ม
        document.getElementById('review-form-title').textContent = `✏️ แก้ไขรีวิวของ: ${r.reviewer_name}`;
        document.getElementById('cancel-review-btn').classList.remove('hidden');
        document.getElementById('review-form-title').scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
...
window.cancelEditReview = () => {
    ...
    document.getElementById('review-name').value = '';
    document.getElementById('review-text').value = '';
    document.getElementById('review-rating').value = '5'; // รีเซ็ตเป็น 5 ดาว
    document.getElementById('review-form-title').textContent = `✨ เพิ่มรีวิวใหม่`;
    document.getElementById('cancel-review-btn').classList.add('hidden');
};
...
window.saveCourseReview = async () => {
    const reviewerName = document.getElementById('review-name').value.trim();
    const reviewText = document.getElementById('review-text').value.trim();
    const reviewRating = document.getElementById('review-rating').value; // ดึงค่าดาวที่เลือก
    ...
    fd.append('reviewer_name', reviewerName);
    fd.append('review_text', reviewText);
    fd.append('rating', reviewRating); // ส่งค่าดาวไปเซิร์ฟเวอร์
...
window.loadCmbData = async () => {
    try {
       ...
       // 🌟 เปิดใช้งานระบบลากวาง (Sortable) สำหรับ CMBigband
        setTimeout(() => {
            const cmbContainer = document.getElementById('cmb-content-container');
            if (window.cmbBuilderSortable) window.cmbBuilderSortable.destroy(); 
            if (cmbContainer) {
                window.cmbBuilderSortable = new Sortable(cmbContainer, {
                    animation: 150, handle: '.block-drag-handle', ghostClass: 'opacity-50'
                });
            }
        }, 300);
...
// =====================================================================
// --- 9. Store & Merch & Ticket Management ---
// =====================================================================
...
window.switchInnerTab = function (tabName) {
    ...
    // รีเซ็ตปุ่ม Ticket
    document.querySelectorAll('#btn-inner-ticket-dashboard, #btn-inner-ticket-order').forEach(btn => {
        btn.classList.remove('bg-blue-600', 'text-white'); btn.classList.add('bg-gray-100', 'text-gray-600');
    });
...
// ดักจับเวลาคลิกเมนูหลักซ้ายมือ เพื่อให้โหลดข้อมูลออโต้
...
// ====== ระบบจัดการ Slip โอนเงิน ======
let uploadSlipOrderId = null; let uploadSlipOrderType = null;

window.viewSlip = (url) => {
    document.getElementById('slip-full-image').src = url;
    document.getElementById('slip-modal').classList.remove('hidden');
};
window.closeSlipModal = () => document.getElementById('slip-modal').classList.add('hidden');

window.triggerSlipUpload = (orderId, type) => {
    uploadSlipOrderId = orderId; uploadSlipOrderType = type;
    document.getElementById('global-slip-upload').click();
};

window.uploadSlipFile = async (input) => {
    if (!input.files || input.files.length === 0) return;
    const fd = new FormData();
    fd.append('order_id', uploadSlipOrderId); fd.append('order_type', uploadSlipOrderType); fd.append('payment_slip_file', input.files[0]);
    try {
...
window.renderStoreOrders = () => {
    ...
    filteredOrders.forEach(o => {
        ...
        tbody.innerHTML += `<tr class="border-b hover:bg-gray-50 align-middle cursor-pointer" onclick="viewOrderDetails('store', ${o.order_id}, event)">
            ...
            <td class="p-3 text-left"><div class="max-w-[150px] line-clamp-2 break-words" title="${o.product_code}">${o.product_code}</div></td>
            <td class="p-3 whitespace-nowrap"><div class="max-w-[120px] truncate" title="${o.customer_name}">${o.customer_name}</div></td>
            <td class="p-3 text-left text-[11px] leading-tight"><div class="max-w-[180px] line-clamp-2 break-words" title="${o.address}">${o.address}</div></td>
            <td class="p-3 whitespace-nowrap">${o.phone}</td>
            <td class="p-3"><div class="max-w-[120px] truncate mx-auto" title="${o.email || '-'}">${o.email || '-'}</div></td>
...
window.renderTicketOrders = () => {
   ...
   filteredOrders.forEach(o => {
        ...
        tbody.innerHTML += `<tr class="border-b hover:bg-gray-50 align-middle cursor-pointer" onclick="viewOrderDetails('ticket', ${o.order_id}, event)">
            <td class="p-3 whitespace-nowrap">${dStr}</td>
            <td class="p-3 font-bold text-gray-700 whitespace-nowrap">${o.order_code}</td>
            <td class="p-3 text-left leading-tight"><div class="max-w-[180px]"><div class="font-bold text-blue-600 truncate" title="${o.event_name || '-'}">${o.event_name || '-'}</div><div class="text-[10px] text-gray-500 truncate" title="${o.ticket_name}">${o.ticket_name}</div></div></td>
            <td class="p-3 whitespace-nowrap"><div class="max-w-[120px] truncate" title="${o.customer_name}">${o.customer_name}</div></td>
            <td class="p-3 whitespace-nowrap">${o.phone}</td>
            <td class="p-3"><div class="max-w-[120px] truncate mx-auto" title="${o.email || '-'}">${o.email || '-'}</div></td>
...
// ==========================================
// 🌟 ระบบจัดการ Store & Merch (Admin Panel)
// ==========================================

let storeProductImages = [];
...
window.previewStoreBanner = function (input) {
    if (input.files && input.files[0]) {
        ...
        const reader = new FileReader();
        reader.onload = (e) => {
            document.getElementById('store-banner-preview').src = e.target.result;
            document.getElementById('store-banner-preview').classList.remove('hidden');
            document.getElementById('store-banner-text').classList.add('hidden');
        }
        reader.readAsDataURL(input.files[0]);
    }
};
        ...
        reader.onload = (e) => {
            container.innerHTML += `
                <div class="relative w-32 h-32 rounded-xl overflow-hidden border border-gray-200 shadow-sm flex-shrink-0 group">
                    ...
                    <button onclick="removeStoreImage(${index})" class="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition shadow">✕</button>
                </div>`;
        };
        reader.readAsDataURL(file);
    });
...
window.allStoreProducts = []; // เพิ่มตัวแปรเก็บข้อมูลสินค้าไว้ด้านนอก
...
window.loadStockData = async function () {
    ...
    try {
        ...
        const result = await res.json();
        if (result.status === 'success') {
            window.allStoreProducts = result.data; // เก็บข้อมูลไว้ใช้ตอนกดแก้ไข
            tbody.innerHTML = '';
            if (result.data.length === 0) {
                ...
                tbody.innerHTML += `
                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                        ...
                        <td class="p-4 text-center space-x-2 whitespace-nowrap">
                            ${badge}
                            <button onclick="editStoreProduct(${p.product_id})" class="ml-2 bg-blue-100 text-blue-600 hover:bg-blue-500 hover:text-white px-4 py-2 rounded-lg font-bold text-sm transition shadow-sm">แก้ไข</button>
                            <button onclick="deleteStoreProduct(${p.product_id})" class="ml-2 bg-red-100 text-red-600 hover:bg-red-500 hover:text-white px-4 py-2 rounded-lg font-bold text-sm transition shadow-sm">ลบ</button>
                        </td>
                    </tr>`;
            });
        }
...
// ==========================================
// 🌟 ฟังก์ชันบันทึกสินค้าใหม่ (Store) แบบส่งรูปได้ 100%
// ==========================================
window.saveNewProduct = async function () {
    ...
    // 🌟 ดึงรูปภาพแบนเนอร์มาส่ง (ถ้ามี)
    if (storeBannerImage) {
        fd.append('image_banner', storeBannerImage);
    }
    
    // 🌟 ดึงรูปภาพสินค้าย่อยมาส่ง (ถ้ามี)
    if (storeProductImages && storeProductImages.length > 0) {
        storeProductImages.forEach(file => {
    ...
    try {
        ...
        try {
            ...
            if (result.status === 'success') {
                showToast('บันทึกสินค้าใหม่สำเร็จ! 🎉');
                
                // เคลียร์ค่าฟอร์ม
                document.getElementById('store_p_name').value = '';
                document.getElementById('store_p_price').value = '';
                document.getElementById('store_p_stock').value = '';
                document.getElementById('store_p_details').value = '';
                ...
                // ซ่อน Preview
                document.getElementById('store-banner-preview').classList.add('hidden');
                document.getElementById('store-banner-text').classList.remove('hidden');
                document.getElementById('store-images-container').innerHTML = `<div class="w-32 h-32 border-2 border-dashed border-gray-400 rounded-xl flex flex-col items-center justify-center text-center text-sm font-bold text-gray-500 cursor-pointer hover:bg-gray-50 transition p-2 flex-shrink-0" onclick="document.getElementById('store-upload-images').click()"><span>+ Add</span><span>Images</span><span class="text-xs font-normal">(Max 5)</span></div>`;
                
                // ย้ายไปหน้าตารางสต๊อก เพื่อดูของใหม่ที่เพิ่งเพิ่ม
                switchInnerTab('stock');
            } else {
                alert("แจ้งเตือน: " + result.message);
            }
        } catch (jsonErr) {
            console.error("PHP Error Log:", text);
        ...
        if (btn) { btn.innerText = 'บันทึกสินค้าใหม่'; btn.disabled = false; } 
    }
};
...
window.viewOrderDetails = function (type, orderId, event) {
   ...
   // สร้างปุ่ม ลบ และ ปิด พร้อมแนบค่า type และ orderId ไปให้ฟังก์ชันลบ
    const actionsContainer = document.getElementById('order-detail-actions');
    if (actionsContainer) {
        actionsContainer.innerHTML = `
            <button onclick="deleteOrder('${type}', ${orderId})" class="px-6 py-2 bg-red-500 text-white font-bold rounded-lg hover:bg-red-600 transition flex items-center gap-2 shadow-sm">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                ลบออเดอร์
            </button>
            <button onclick="document.getElementById('order-detail-modal').classList.add('hidden')" class="px-6 py-2 bg-gray-200 text-gray-800 font-bold rounded-lg hover:bg-gray-300 transition shadow-sm">ปิด</button>
        `;
    }
...
// ฟังก์ชันลบออเดอร์
window.deleteOrder = async function(orderType, orderId) {
    if (!confirm('🚨 คุณแน่ใจหรือไม่ที่จะลบออเดอร์นี้?\n(หากมีรูปสลิป รูปจะถูกลบออกจากเซิร์ฟเวอร์ด้วย)')) return;
    const fd = new FormData(); fd.append('order_id', orderId); fd.append('order_type', orderType);
    try {
        const res = await fetch(getApiUrl('delete_order'), fetchOptions('POST', fd));
        const result = await res.json();
        if (result.status === 'success') {
            showToast('ลบออเดอร์เรียบร้อยแล้ว');
            document.getElementById('order-detail-modal').classList.add('hidden');
            if (orderType === 'store') loadOrderData(); else loadTicketOrderData();
        } else { showToast('Error: ' + result.message); }
    } catch (e) { showToast('ไม่สามารถเชื่อมต่อเซิร์ฟเวอร์ได้'); }
};

// กู้คืนระบบดึงดาวมารอแก้
document.getElementById('btn-action-edit-review')?.addEventListener('click', () => {
    if (!selectedReviewId) return;
    const r = allCourseReviews.find(x => x.id == selectedReviewId);
    if (r) {
        ...
        document.getElementById('review-name').value = r.reviewer_name;
        document.getElementById('review-text').value = r.review_text;
        document.getElementById('review-rating').value = r.rating || 5; 
        document.getElementById('review-form-title').textContent = `✏️ แก้ไขรีวิวของ: ${r.reviewer_name}`;
        document.getElementById('cancel-review-btn').classList.remove('hidden');
        document.getElementById('review-form-title').scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
...
window.saveCourseReview = async () => {
    ...
    if (!reviewerName || !reviewText) return showToast('กรุณากรอกข้อมูลให้ครบ');

    const fd = new FormData();
    fd.append('review_id', document.getElementById('edit-review-id').value);
    fd.append('reviewer_name', reviewerName); fd.append('review_text', reviewText); fd.append('rating', reviewRating);

    try {
        const res = await fetch(getApiUrl('save_course_review'), fetchOptions('POST', fd));
        const result = await res.json();
        showToast(result.message);
        if (result.status === 'success') { fetchCourseReviews(); cancelEditReview(); }
    } catch (e) { }
};
// ==========================================
// 🌟 ฟังก์ชันลบออเดอร์ (ใช้ได้ทั้ง Store และ Ticket)
// ==========================================
window.deleteOrder = async function(orderType, orderId) {
    if (!confirm('🚨 คุณแน่ใจหรือไม่ที่จะลบออเดอร์นี้?\n(หากมีรูปสลิป รูปจะถูกลบออกจากเซิร์ฟเวอร์ด้วย และไม่สามารถกู้คืนได้)')) return;
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            // รีโหลดตารางให้ข้อมูลหายไปทันที
            if (orderType === 'store') {
                loadOrderData();
            } else {
                loadTicketOrderData();
...
// ==========================================
// 🌟 ฟังก์ชันจัดการหน้าแก้ไขสินค้า Store
// ==========================================
let editStoreProductImages = [];
let editStoreBannerImage = null;
let existingStoreImages = [];

window.editStoreProduct = function (id) {
    const p = window.allStoreProducts.find(item => item.product_id == id);
    if (!p) return;

    // นำข้อมูลไปใส่ในฟอร์มแก้ไข
    document.getElementById('edit_store_p_id').value = p.product_id;
    document.getElementById('edit_store_p_name').value = p.name;
    document.getElementById('edit_store_p_price').value = p.price;
    document.getElementById('edit_store_p_stock').value = p.stock_balance;
    ...
    if (p.image_banner) {
        bannerPreview.src = p.image_banner;
    ...
    // จัดการรูปสินค้าย่อยเดิม
    if (p.image_products) {
        try { existingStoreImages = JSON.parse(p.image_products); } catch (e) {}
    }
...
window.previewEditStoreBanner = function (input) {
    if (input.files && input.files[0]) {
        ...
        const reader = new FileReader();
        reader.onload = (e) => {
            document.getElementById('edit-store-banner-preview').src = e.target.result;
            document.getElementById('edit-store-banner-preview').classList.remove('hidden');
...
window.handleEditStoreImages = function (input) {
    if (input.files) {
        const totalCurrent = existingStoreImages.length + editStoreProductImages.length;
        const remainingSlots = 5 - totalCurrent;
        const filesToAdd = Array.from(input.files).slice(0, remainingSlots);
        filesToAdd.forEach(file => editStoreProductImages.push(file));
        renderEditStoreImagePreviews();
    }
    input.value = '';
};
...
window.renderEditStoreImagePreviews = function () {
    const container = document.getElementById('edit-store-images-container');
    container.innerHTML = `<div class="w-20 h-20 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center text-center text-xs font-bold text-gray-400 cursor-pointer hover:bg-gray-100 transition p-1 flex-shrink-0" onclick="document.getElementById('edit-store-upload-images').click()"><span>+ Add</span><span>(Max 5)</span></div>`;

    existingStoreImages.forEach((imgUrl, index) => {
        container.innerHTML += `
            <div class="relative w-20 h-20 rounded-lg overflow-hidden border border-gray-200 shadow-sm flex-shrink-0 group">
                <img src="${imgUrl}" class="w-full h-full object-cover">
                <button type="button" onclick="removeExistingStoreImage(${index})" class="absolute top-0 right-0 bg-red-500 text-white rounded-bl-lg w-5 h-5 flex items-center justify-center text-[10px] opacity-0 group-hover:opacity-100 transition shadow">✕</button>
            </div>`;
    });

    editStoreProductImages.forEach((file, index) => {
        const reader = new FileReader();
        reader.onload = (e) => {
            container.innerHTML += `
                <div class="relative w-20 h-20 rounded-lg overflow-hidden border border-gray-200 shadow-sm flex-shrink-0 group">
                    <img src="${e.target.result}" class="w-full h-full object-cover opacity-80">
                    <span class="absolute bottom-0 w-full text-center bg-blue-500 text-white text-[8px] py-0.5">NEW</span>
                    <button type="button" onclick="removeNewEditStoreImage(${index})" class="absolute top-0 right-0 bg-red-500 text-white rounded-bl-lg w-5 h-5 flex items-center justify-center text-[10px] opacity-0 group-hover:opacity-100 transition shadow">✕</button>
                </div>`;
        };
        reader.readAsDataURL(file);
    });
...
window.removeExistingStoreImage = function (index) {
    existingStoreImages.splice(index, 1); renderEditStoreImagePreviews();
};
window.removeNewEditStoreImage = function (index) {
    editStoreProductImages.splice(index, 1); renderEditStoreImagePreviews();
};
...
window.saveEditedStoreProduct = async function () {
    ...
    const fd = new FormData();
    fd.append('product_id', id); fd.append('product_name', name);
    fd.append('product_price', price); fd.append('product_stock', stock);
    fd.append('product_details', details); fd.append('sale_status', status);

    existingStoreImages.forEach(img => fd.append('existing_images[]', img));
    if (editStoreBannerImage) fd.append('image_banner', editStoreBannerImage);
    editStoreProductImages.forEach(file => fd.append('product_images[]', file));

    try {
        showToast('กำลังบันทึกข้อมูลแก้ไข...');
        const res = await fetch(getApiUrl('update_store_product'), fetchOptions('POST', fd));
        const result = await res.json();
        
        if (result.status === 'success') {
            showToast('แก้ไขข้อมูลสินค้าสำเร็จ! 🎉');
