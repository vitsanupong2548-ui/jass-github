...
// ==========================================
// 🌟 0. Dictionary สำหรับแปลหน้าเว็บหลัก (HTML UI) 🌟
// ==========================================

const siteTranslations = {
    en: {
        // --- 1. Navbar, Hero Section & Search ---
        "nav_book": "Book Now",
        "nav_login": "Log in",
        ...
        // --- 3. Popup Panels (เนื้อหาฝั่งซ้ายของแต่ละหน้า) ---
        ...
        // --- 4. Sub-Contents (เนื้อหาย่อยด้านใน / รายละเอียด) ---
    ...
    th: {
        ...
        "forum_sub_title": "คอมมูนิตี้พูดคุย",
        "forum_sub_desc": "เข้ามาตั้งคำถาม แชร์ความรู้ และเมคเฟรนด์กับชาวล้านนา-แจ๊สได้เลย",
...
// 3. ฟังก์ชันแปลภาษา (อัปเดตใหม่ให้จับ Element ที่เพิ่งถูกโคลนขึ้นมาอัตโนมัติ)
window.translateUI = function(root = document) {
    ...
    // แปลปุ่มและ UI หลัก (ทำเฉพาะตอนอ้างอิงถึง document)
    ...
    // 🌟 แปล Dynamic Elements ทั้งหมดที่อยู่ใน container ที่กำหนด
    const dynamicElements = container.querySelectorAll('[id^="dyn-"]');
    dynamicElements.forEach(el => {
        let rawKey = el.id.replace('dyn-', ''); 
        let key = rawKey;
        
        // ถ้าหาคีย์ตรงๆ ไม่เจอ (เพราะติด Suffix ตอนถูกโคลน เช่น _lib) ให้ลองตัด Suffix ออก
        if (!t[key] && key.includes('_')) {
            key = key.replace(/_[^_]+$/, ''); // ตัด _ และข้อความข้างหลังตัวสุดท้ายออก
        }
...
// 🌟 ตัวแอบสังเกตการณ์ (MutationObserver) 
// ถ้ามีโค้ด HTML อะไรถูกเด้งหรือโคลนขึ้นมาบนหน้าเว็บใหม่ ให้สั่งแปลภาษาแบบเรียลไทม์ทันที!
const observer = new MutationObserver((mutations) => {
    mutations.forEach(mutation => {
        mutation.addedNodes.forEach(node => {
            if (node.nodeType === 1) { // ถ้าเป็น Element HTML
...
// สั่งให้จับตาดูโครงสร้างเว็บไซต์ตลอดเวลา
