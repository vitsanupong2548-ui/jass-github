...
async function loadFrontendMusicians() {
    try {
        ...
        if (result.status === 'success') {
            ...
            window.frontendMusicians = musicians; 
            ...
            if (artistContainer) {
                ...
                artistLibrary.forEach((artist, index) => {
                    ...
                    const displayTitle = artist.title || 'Untitled';
            ...
            if (jazzContainer) {
                ...
                jazzNetwork.forEach((jazz, index) => {
                    ...
                    const displayTitle = jazz.title || 'Untitled';
    ...
    setTimeout(() => {
        ...
        if (musicianId && window.frontendMusicians) {
            ...
            if (musician) {
                ...
                const genre = musician.genre ? `<p class="italic text-gray-700 font-medium mb-3 break-words break-all whitespace-normal">${musician.genre}</p>` : '';
                ...
                const detailsHtml = musician.details ? `<div class="break-words break-all whitespace-normal">${musician.details.replace(/\n/g, '<br>')}</div>` : '';
                ...
                if (result.status === 'success' && result.data.length > 0) {
                    ...
                    events.forEach((event) => {
                        ...
                        if(hiddenContainer) {
                            ...
                            if (event.venue_details) venueHTML += `<div class="prose prose-sm sm:prose-base max-w-none text-gray-700 font-medium leading-relaxed mb-6 whitespace-pre-line">${event.venue_details}</div>`;
                            if (event.venue_map) {
        ...
        if (rightPane) {
            try {
                ...
                if (result.status === 'success' && result.data.length > 0) {
                    ...
                    window.frontendCourses = courses;

                    // หยอดข้อมูลกล่องหลักใหญ่สุด
                    if (courses[0]) {
                        ...
                        if (bigTitle) bigTitle.textContent = courses[0].title || 'Untitled Course';
                        if (bigCreator) bigCreator.textContent = `By ${courses[0].creator || 'Unknown'}`; 
                        if (bigLink) bigLink.setAttribute('data-course-index', courses[0].id); 
                    }

                    // 🌟 จัดการกล่องเล็กด้านล่าง (ทำเป็นระบบสไลด์เลื่อนซ้าย-ขวา) 🌟
        ...
        if (gridView && detailView && bigbandLink) {
            try {
                ...
                if (result.status === 'success' && result.data) {
                    ...
                    if (bbData.details) {
                        try {
                            let detailsArray = typeof bbData.details === 'string' ? JSON.parse(bbData.details) : bbData.details;
                            if (Array.isArray(detailsArray)) {
                                detailsArray.forEach(item => {
                                    let widthClass = 'w-full'; 
                                    if (item.layout === 'col-2') widthClass = 'w-1/2'; 
                    ...
                    const title = bbData.title || 'CMSJ Bigband';
                    const genre = bbData.genre || '';
            ...
            } catch (error) { console.error('Error fetching bigband:', error); }
        }
    }
};