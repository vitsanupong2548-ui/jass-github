...
document.addEventListener('DOMContentLoaded', async () => {
 ...
 // ==========================================
// 🌟 ระบบปุ่ม Learn More วิ่งทะลุไปหน้า Bigband Detail ทันที
// ==========================================
document.addEventListener('click', function(e) {
    const learnMoreBtn = e.target.closest('#btn-learn-more');
    ...
    if (learnMoreBtn) {
        ...
        const activeCloseBtn = document.querySelector('.card.clone .close-btn');
        if (activeCloseBtn) activeCloseBtn.click(); 

        // 🌟 ตั้ง Flag ว่าให้เปิด Detail ทันทีที่ API โหลดข้อมูลเสร็จ
        window.autoOpenBigbandDetail = true;

        setTimeout(() => {
            const bigbandCard = document.querySelector('.card[data-target="#bigband-content"]');
            if (bigbandCard) bigbandCard.click(); 
        }, activeCloseBtn ? 600 : 400); 
    }
});
            ...
            try {
                ...
                window.frontendEvents.forEach((event) => {
                    ...
                    const imageUrl = event.banner_image ? event.banner_image : 'https://placehold.co/1200x400/1a1a1a/ffffff?text=Jazz+Event';
                        // 🌟 สร้างตัวแปรแปลภาษาแบบ Dynamic
                    const txtGoBack = window.currentLang === 'th' ? 'กลับ' : 'Go Back';
                    const txtNoDetail = window.currentLang === 'th' ? 'ไม่มีรายละเอียด...' : 'No details...';
                    const txtNoTicket = window.currentLang === 'th' ? 'ไม่มีข้อมูลบัตรเข้าชม' : 'No tickets available.';
                    const txtAgree = window.currentLang === 'th' ? 'ข้าพเจ้ายินยอมให้ข้อมูลกับผู้จัดงาน' : 'By checking this box, I hereby agree that my information will be shared to our Event Organizers';
                    const txtBuyTicket = window.currentLang === 'th' ? 'สั่งซื้อบัตร' : 'BUY TICKET';
                    const txtScanPay = window.currentLang === 'th' ? 'สแกนเพื่อชำระเงิน' : 'Scan to Pay';
                    const txtScanDesc = window.currentLang === 'th' ? 'กรุณาสแกนคิวอาร์โค้ดเพื่อชำระเงิน' : 'Please scan the QR code to complete your payment.';
                    const txtTotal = window.currentLang === 'th' ? 'ยอดรวม:' : 'Total Amount:';
                    const txtName = window.currentLang === 'th' ? 'ชื่อ-นามสกุล' : 'Full Name';
                    const txtPhone = window.currentLang === 'th' ? 'เบอร์โทรศัพท์' : 'Phone Number';
                    const txtSlip = window.currentLang === 'th' ? 'อัปโหลดสลิป' : 'Upload Slip';
                    const txtCancel = window.currentLang === 'th' ? 'ยกเลิก' : 'Cancel';
                    const txtConfirm = window.currentLang === 'th' ? 'ยืนยัน' : 'Confirm';
                    const txtNoLineup = window.currentLang === 'th' ? 'ไม่มีตารางศิลปิน' : 'No lineup schedule.';
                    const txtEvGal = window.currentLang === 'th' ? 'แกลลอรี่ภาพ' : 'Event Gallery';
                    const txtNoGal = window.currentLang === 'th' ? 'ยังไม่มีรูปภาพ' : 'No gallery images.';
                    const cardHTML = `
                    ...
                    if (hiddenContainer) {
                        ...
                        if (event.tickets && event.tickets.length > 0) {
                          event.tickets.forEach(t => {
                                let stockAmount = parseInt(t.amount) || 0;
                                let isOpen = parseInt(t.is_open) === 1; // เช็คสถานะ 1 = เปิดขาย, 0 = ปิดขาย
                                let ticketControls = '';
                                
                                // คำแปลสำหรับแสดงจำนวนและป้าย Sold Out
                                let leftText = window.currentLang === 'th' ? `เหลือ ${stockAmount} ใบ` : `${stockAmount} left`;
                                let soldOutText = window.currentLang === 'th' ? 'บัตรหมด / ปิดการขาย' : 'SOLD OUT / CLOSED';

                                // ถ้าบัตรยังเหลือ และ แอดมินเปิดการขายอยู่ (Sale Open) ให้แสดงปุ่ม + / -
                                if (stockAmount > 0 && isOpen) {
                                    ticketControls = `
                                        <div class="flex items-center border border-black rounded bg-white">
                                            ...
                                            <span class="ticket-qty-val w-10 text-center font-bold" data-max="${stockAmount}">0</span>
                                    ...
                                    // ถ้าบัตรหมด (0) หรือ แอดมินตั้งค่าเป็น Sale Close ให้แสดงป้ายแดง และกดซื้อไม่ได้
                                    ticketControls = `<span class="bg-red-500 text-white font-bold px-4 py-2 rounded-full text-[10px] sm:text-xs tracking-widest shadow-sm whitespace-nowrap">${soldOutText}</span>`;
                                }
                            ...
                            ticketsHTML += `<div class="mt-6 flex flex-col gap-6"><label class="flex items-start gap-3 cursor-pointer"><input type="checkbox" class="chk-ticket-agree mt-1 w-4 h-4 rounded border-gray-300 text-black focus:ring-black"><span class="text-[10px] sm:text-xs text-black font-medium leading-relaxed">${txtAgree}, I hereby agree that my information will be shared to our Event Organizers</span></label><button class="btn-buy-ticket w-full bg-black text-white font-header font-bold text-xl py-4 rounded-full tracking-wider hover:bg-gray-800 transition-colors uppercase shadow-lg" data-event-id="${event.id}">${txtBuyTicket}</button></div>`;
                        } else {
                            ticketsHTML += '<p class="text-gray-500 font-medium mt-4">txtNoTicket</p>';
                        }
                        ticketsHTML += '</div>';
                        ...
                        <div class="ticket-payment-section hidden flex flex-col items-center justify-center bg-[#f8f9fa] p-6 md:p-10 rounded-2xl shadow-sm border border-gray-200 w-full text-center transition-opacity duration-300">
                            <h3 class="text-2xl md:text-3xl font-header font-bold text-black mb-2">${txtScanPay}</h3>
                            <p class="text-gray-500 mb-6 text-sm font-medium">${txtScanDesc}</p>
                            
                            <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 mb-6">
                                <img src="icon/qrcode.webp" alt="QR Code" class="w-80 h-120 md:w-100 md:h-100 object-contain">
                            </div>
                            
                            <div class="bg-black text-white px-8 py-3 rounded-full mb-8 shadow-md">
                                <span class="font-medium mr-2">${txtTotal}</span>
                            ...
                            <form class="w-full max-w-sm text-left space-y-4 mb-8">
                                <div>
                                    <label class="block text-xs font-bold text-gray-700 mb-1 uppercase">${txtName}</label>
                                ...
                                <div>
                                   <label class="block text-xs font-bold text-gray-700 mb-1 uppercase">${txtPhone}</label>
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-gray-700 mb-1 uppercase">${txtSlip}</label>
                            ...
                            <div class="flex gap-4 w-full max-w-sm">
                                <button type="button" class="btn-cancel-payment flex-1 bg-gray-300 text-black font-bold py-3.5 rounded-full hover:bg-gray-400 transition shadow-sm">${txtCancel}</button>
                                <button type="button" class="btn-confirm-payment flex-1 bg-[#10a349] text-white font-bold py-3.5 rounded-full hover:bg-green-700 transition shadow-lg">${txtConfirm}</button>
                            </div>
                        </div>
                        `;
                        ...
                        if (event.lineups && event.lineups.length > 0) {
                            ...
                            lineupHTML += '</div>';
                        } else { lineupHTML = '<div class="p-8 text-center text-gray-500">' + txtNoLineup + '</div>'; }
                        ...
                        if (event.gallery_images) {
                            try {
                                ...
                                else { galleryHTML += '<p class="col-span-3 text-gray-500 font-medium">+ txtNoGal +</p>'; }
                        ...
                        galleryHTML += '</div>';
                        galDiv.innerHTML = `<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">${leftCol}<div class="lg:col-span-8"><h2 class="text-3xl font-header font-bold mb-2">${txtEvGal}</h2>${galleryHTML}</div></div>`;
                    }
                });
    ...
    function renderCourseReviews() {
        ...
        if (window.courseReviewsData.length === 0) {
           ...
           paginatedItems.forEach(review => {
                ...
                // ดึงคะแนนจากฐานข้อมูลมาแสดงเป็นดาวแบบ Dynamic
                const ratingCount = parseInt(review.rating) || 5;
                let starsText = '';
                for (let i = 0; i < 5; i++) { starsText += i < ratingCount ? '★' : '☆'; }
                const starsHtml = `<div class="text-yellow-400 text-lg tracking-widest mb-1 drop-shadow-sm">${starsText}</div>`;
        ...
        if (gridView && detailView && bigbandLink) {
            try {
                ...
                if (result.status === 'success' && result.data) {
                    ...
                    }); // <--- ปิด newLink.addEventListener

                    // 🌟 เพิ่มโค้ด 4 บรรทัดนี้ต่อท้ายทันที 🌟
                    if (window.autoOpenBigbandDetail) {
                        window.autoOpenBigbandDetail = false; // เคลียร์สถานะ
                        setTimeout(() => newLink.click(), 150); // สั่งกดทะลุเข้าไปเลย
                    }
...
// เตรียมข้อมูลส่งไป Backend ให้ชื่อตัวแปรตรงกับที่ PHP กำหนดไว้
        const fd = new FormData();
        ...
        fd.append('cart_items', cart); // เปลี่ยนให้ตรงกับ backend
        ...
        cart.forEach(item => {
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-500 font-medium">${window.currentLang === 'th' ? 'จำนวน' : 'Qty'}: ${item.qty}</p>
                </div>
...
document.addEventListener('click', function (e) {
    ...
    // 2. ปุ่มเพิ่มจำนวนตั๋ว (+) และล็อกจำนวนสูงสุด
    if (e.target.closest('.btn-ticket-plus')) {
        ...
        if (wrapper) {
            const qtySpan = wrapper.querySelector('.ticket-qty-val');
            let qty = parseInt(qtySpan.innerText) || 0;
            let maxStock = parseInt(qtySpan.getAttribute('data-max')) || 0; // ดึงโควตาที่มี
            
            if (qty < maxStock) {
                qtySpan.innerText = qty + 1;
            } else {
                window.showCustomAlert(window.currentLang === 'th' ? `ขออภัย บัตรประเภทนี้เหลือเพียง ${maxStock} ใบครับ` : `Sorry, only ${maxStock} tickets available.`);
            }
        }
    }
                ...
                .then(result => {
                    if (result.status === 'success') {
                        window.showCustomAlert(window.currentLang === 'th' ? '✅ ชำระเงินและส่งหลักฐานสำเร็จ! ขอบคุณสำหรับการสั่งซื้อครับ' : '✅ Payment and slip submitted successfully!', () => {
                            // 🌟 สั่งให้รีเฟรชหน้าเว็บ 1 ครั้ง เมื่อกดปุ่ม OK ปิดแจ้งเตือน
                            window.location.reload(); 
                        });
                    } else {
                        window.showCustomAlert('Error: ' + result.message);
